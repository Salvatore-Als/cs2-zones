/**
Source: https://github.com/FUNPLAY-pro-CS2/Ray-Trace/blob/main/public/Example.cs
*/
...
#region Native Structs (matching C++ layout exactly)
	...
	public struct TraceOptions
	{
		...
		/// <summary>Options avec un seul masque (InteractsAs et InteractsWith). Ex: new TraceOptions(InteractionLayers.MASK_SHOT_FULL)</summary>
		public TraceOptions(InteractionLayers mask)
		{
			InteractsAs = (ulong)mask;
  ...
  public struct TraceResult
  {
	...
	public static class CRayTrace
	{
		...
		{
			object? factory = Utilities.MetaFactory("CRayTraceInterface001");
			if (factory == null)
				throw new InvalidOperationException("Ray-Trace interface not found. Is the Ray-Trace MetaMod module loaded?");

			g_pRayTraceHandle = (nint)factory;
			if (g_pRayTraceHandle == nint.Zero)
				throw new InvalidOperationException("Ray-Trace interface handle is null. Is the Ray-Trace MetaMod module loaded?");
